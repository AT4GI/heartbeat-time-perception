"""
time_reproduction_precision.py
-------------------------------
精度重視版（TRIALS_PER_CELL=3、時間に余裕のある参加者向け）

time_reproduction.py（TRIALS_PER_CELL=2、短時間版）の複製。
分岐ロジックはコード内に持たせず、実験者が起動時にどちらのファイルを
実行するかで版を選ぶ。
...
TRIALS_PER_CELL = 3                        # 各条件×各時間の試行数（精度重視版）
...
def save_trial(filename, participant_name, condition, target, reproduced,
    ...
    with open(filename, "a", newline="", encoding="utf-8") as f:
        ...
        writer.writerow([
            ...
            trial_num, block_num, is_practice,
            datetime.now().strftime("%Y%m%d_%H%M%S.%f"), "precision"
        ])
...
def main():
    ...
    # 精度重視版であることをファイル名から分かるようにする。
    # setup_data_file()だけでなく、screening.py/debrief.py側の
    # ファイル名もこのparticipant_info["date"]から組み立てられるため、
    # ここで一度サフィックスを付ければ sub-*.csv / screening-*.csv /
    # debrief-*.csv すべてに反映され、短時間版のCSVと混ざらない
    # （screening.py/debrief.py自体は変更しない）。
    participant_info["date"] = participant_info["date"] + "_precision"

    # 参加者番号の自動採番（"_precision"を含むファイルのみをカウント。
    # 短時間版time_reproduction.pyの採番とは独立）
