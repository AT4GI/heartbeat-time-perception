...
def _ask_all_debrief_questions():
    """全質問を1つのダイアログにまとめて尋ねる（デフォルトは「いいえ」）。"""
...
def run_debrief(win, participant_info, data_dir):
    ...
    responses = {}
    for (item_id, jp, en), value in zip(DEBRIEF_ITEMS, answers):
        responses[item_id] = "y" if value == YES_LABEL else "n"

    win.winHandle.activate()
    _save_debrief(participant_info, responses, data_dir)
...
def _save_debrief(participant_info, responses, data_dir):
