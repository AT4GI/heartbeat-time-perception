...
expName = 'psychopy_slider_test'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\mets8_01ua6fp\\Downloads\\研究室\\心拍\\psychopy\\psychopy_test\\psychopy_slider_test_lastrun.py',
...
def setupWindow(expInfo=None, win=None):
    ...
    """
    if PILOTING:
        logging.debug('フルスクリーン設定はpilotモードでの実行時には無視されます.')
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    question_text = visual.TextStim(win=win, name='question_text',
        ...
        languageStyle='LTR',
        depth=0.0);
    rating_time_long = visual.Slider(win=win, name='rating_time_long',
        startValue=None, size=(1.1, 0.08), pos=(0, -0.15), units=win.units,
    ...
    label_left = visual.TextStim(win=win, name='label_left',
        text=None,
        font='Arial',
        pos=(-0.42, -0.38), draggable=False, height=0.045, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-2.0);
    ...
    trial = data.Routine(
        name='trial',
        components=[question_text, rating_time_long, label_left, label_right],
    )
    ...
    while continueRoutine:
        ...
        # if rating_time_long is starting this frame...
        if rating_time_long.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            rating_time_long.frameNStart = frameN  # exact frame index
            rating_time_long.tStart = t  # local t and not account for scr refresh
            rating_time_long.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(rating_time_long, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'rating_time_long.started')
            # update status
            rating_time_long.status = STARTED
            rating_time_long.setAutoDraw(True)
        
        # if rating_time_long is active this frame...
        if rating_time_long.status == STARTED:
            # update params
            pass
        
        # Check rating_time_long for response to end Routine
        if rating_time_long.getRating() is not None and rating_time_long.status == STARTED:
    ...
    thisExp.addData('rating_time_long.response', rating_time_long.getRating())
    thisExp.addData('rating_time_long.rt', rating_time_long.getRT())
    thisExp.nextEntry()
